...
M.slug = function(file)
    ...
    local result = vim
        .system({ "git", "-C", vim.fn.fnamemodify(file, ":p:h"), "remote", "get-url", "origin" }, { text = true })
        :wait()
...
M.path = function(file)
    ...
    local result = vim
        .system({ "git", "-C", vim.fn.fnamemodify(file, ":p:h"), "rev-parse", "--show-toplevel" }, { text = true })
        :wait()
    if result.code ~= 0 or not result.stdout or result.stdout == "" then
